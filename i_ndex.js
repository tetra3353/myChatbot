...
io.on('connection', function(socket){
  socket.on('chat message', function(msg){
    ...
    //このページにアクセスした時に、からのメッセージをConversationに投げる
    ...
    function processResponse(err,response){
      ...
      //ユーザーの入力文を取得
      var myQuery =response.input.text;
      console.log('１');
  ...
  //  highlight: true // if you want to enable, uncomment
      ...
      if (!response.output){
        console.log('root1..');
        response.output={};
      }else if (response.output.action==='call_discovery'){
        ...
        discovery.query(params,(error,results)=>{
          if (error){
            ...
            io.to(socket.id).emit('bot message',results.passages[0].passage_text);
          };
        });
      }else if (response.outpt && response.output.text){
        io.to(socket.id).emit('bot message',response.output.text[0]);
      }
